...
function waGetSourceCode() {
    ...
    if [[ ! -d "$SOURCE_PATH" ]]; then
        $SUDO git clone --recurse-submodules --remote-submodules https://github.com/v3DJG6GL/winapps.git "$SOURCE_PATH"
    else
...
function waGetInquirer() {
    ...
    if [ -d "$SYS_SOURCE_PATH" ]; then
        ...
        curl -o "$INQUIRER" "https://raw.githubusercontent.com/v3DJG6GL/winapps/main/install/inquirer.sh"
    fi
